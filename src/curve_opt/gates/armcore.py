...
-------------------------
:class:`Input.broadcast` reads the module-level ``T`` (= ``DEV.gate_time``), not a
per-instance one, exactly as it did in F05.  Every gate in this project shares the
50 ns duration, so this is currently a distinction without a difference -- but if a
gate ever runs at another duration, this is the line to fix, and it is a genuine
limitation rather than an accident of where the code sits.
"""
...
from curve_opt import basis, budget, device, parametrization, propagate
...
class Input:
    ...
    def broadcast(self, N: int = N_DESIGN):
        ...
        if self.layer == "planar":
            return parametrization.planar_waveform(self.a, T, N)
        if self.layer == "planar_drag":
            return parametrization.planar_drag_waveform(self.a, T, DEV.delta, N)
        if self.layer == "general":
            return parametrization.general_waveform(self.a, self.c, self.Phi_0, T,
                                                    DEV.delta, N)
    ...
    def predicted_terms(self, N: int = N_DESIGN) -> dict:
        ...
        **broadcast (played)** waveform.
        ...
        dchain = budget.design_chain(self.a, self.c, self.Phi_0, T, N)
        ...
        bchain = propagate.chain(om_x, om_y, T)
        Lambda = np.asarray(propagate.leakage_amplitude(bchain, DEV.delta))
