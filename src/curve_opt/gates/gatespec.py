...
from curve_opt import device, propagate

HERE = Path(__file__).resolve().parent
REPO_ROOT = HERE.parents[2]   # src/curve_opt/gates -> repo root
...
class GateSpec:
    ...
    folder: str
    """Export directory **name** -- not a path.

    Resolved to ``results/<folder>/`` by :func:`gate_dir` (created on demand).
    The name is kept identical to the historical proposal-folder name so the
    golden baseline files and every recorded manifest keep matching.
    """
...
def gate_dir(sp: "GateSpec | str") -> Path:
    """The export/workspace directory for a gate: ``results/<folder>/``, on demand.

    Until F09 this searched the git-ignored ``proposals/`` tree for the notebook
    folder of the same name. The clean tree carries no proposal folders, so the
    directory is simply where :meth:`gatelib.GateLab.export_arms` writes waveforms
    and ``manifest.json``; it is created the first time a gate asks for it.
    """
    name = sp if isinstance(sp, str) else sp.folder
    if name in _DIR_CACHE:
        return _DIR_CACHE[name]
    d = REPO_ROOT / "results" / name
