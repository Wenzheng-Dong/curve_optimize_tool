...
This is ``1qb_Gaussian-drag-optimization/armlib.py`` with the target gate lifted
out of module scope and into a :class:`gatespec.GateSpec`.  Each proposal folder
instantiates one :class:`GateLab` and re-exports its methods from a three-line
``armlib.py`` shim, so the four notebooks stay nearly identical while the gate
...
# ``_dev_logs`` is deliberately *not* on this path any more (F08c): the proposal
# layer no longer imports anything from the frozen dev-log archive.
for _p in (REPO_ROOT / "src", str(HERE)):
...
from curve_opt import basis, budget, device, gate, geometry, parametrization, propagate, qutip_sim  # noqa: E402
import armcore  # noqa: E402

import gatespec  # noqa: E402
from gatespec import DEV, GateSpec, REPO_ROOT as _RR, SPECS, T, rotate_drive, source_spec  # noqa: E402,F401
from armspec import (  # noqa: E402
...
def bind(lab: "GateLab", ns: dict) -> "GateLab":
    ...
    for name in ("run_dir", "trace_run_dir",
                 ...
                 "characterize_zero_noise", "characterize", "qutip_cross_check",
...
class GateLab:
    ...
    def __init__(self, spec: GateSpec, n_design: int = N_DESIGN, n_sim: int = N_SIM,
        ...
        """*device* defaults to the module-level :data:`DEV` (``None``) -- the default
        path is unchanged bit for bit (F08_golden.py). Pass a non-default
        :class:`curve_opt.device.Device` (e.g. loaded via
        ``curve_opt.recorder.load_device``) to build every arm and evaluate every
        cost against that device instead (N1)."""
        self.spec = spec
        self.DEV = DEV if device is None else device
        self.T = self.DEV.gate_time
        self.theta = float(spec.theta)
        self.phi_axis = float(spec.phi_axis)
        self.U_TARGET = spec.U_target
        self.FLUENCE_FLOOR = spec.fluence_floor(self.DEV)
        self.N_DESIGN = n_design
        self.N_SIM = n_sim
        # Same expression as the module-level N_EXPORT (line ~80), evaluated against
        # *this* device so a non-default sample_rate/gate_time is honoured too; bit
        # identical to the module constant when device is None (the default path).
        self.N_EXPORT = int(round(self.DEV.sample_rate * self.DEV.gate_time))
    ...
    def build_gaussian_drag(self) -> armcore.Input:
        ...
        S = basis.design_matrix(t, self.T, self.DEV.n_modes)
        ...
        return armcore.Input(
            rec.name, rec.layer, a, np.zeros(self.DEV.n_modes), 0.0,
            f"Truncated Gaussian (sigma=T/{rec.sigma_divisor:g}) for "
            f"theta={self.theta:.6f}, least-squares "
            f"fit to M={self.DEV.n_modes} sine modes (relRMS after c1 projection = "
    ...
    def build_arm_a(self, arm_b: armcore.Input) -> armcore.Input:
        ...
        return armcore.Input(
            ...
            f"fitted to M={self.DEV.n_modes} sine modes, "
    ...
    def solve_arm(self, arm_key: str, *, run_id: str | None = None,
        ...
        *warm_arm* defaults to the recipe's warm start resolved through
        ...
        c0 = (np.zeros(self.DEV.n_modes) if warm_arm.layer == "planar_drag"
        ...
        c1_res = np.array([basis.c1_row(self.DEV.n_modes, self.T, "start") @ np.asarray(arm.a),
                           basis.c1_row(self.DEV.n_modes, self.T, "end") @ np.asarray(arm.a)])
        ...
        if verbose:
            ...
            print(f"peak_phys     = {result.peak_phys:.6f} "
                  f"({result.peak_phys / self.DEV.rabi_max_rate:.1%} of Omega_max)", flush=True)
    ...
    def geometry_of(self, arm, N: int | None = None) -> dict:
        ...
        return {
            ...
            "tau_guard_bound": 0.5 * abs(self.DEV.delta) / 2.0,
    ...
    def characterize_zero_noise(self, arm, N: int | None = None) -> dict:
        ...
        U3 = np.asarray(gate.three_level_propagator(om_x, om_y, self.T, self.DEV.delta))
    ...
    def characterize(self, arm, N: int | None = None) -> dict:
        ...
        row["peak"] = peak
        row["peak_frac"] = peak / self.DEV.rabi_max_rate
        row["decoherence_floor"] = self.DEV.decoherence_floor
        row["predicted_plus_decoh"] = row["total"] + self.DEV.decoherence_floor
        row["true_plus_decoh"] = row["baseline_infidelity"] + self.DEV.decoherence_floor
        row["ratio_true_over_predicted"] = row["baseline_infidelity"] / row["total"]
        return row

    def qutip_cross_check(self, arm, N: int = 1200) -> dict:
        """Independent QuTiP reading of the same waveform (adaptive ODE, no ``curve_opt.gate``).

        A jax/QuTiP disagreement beyond ~1e-4 relative at this grid is a bug, not a result.
        """
        t_mid, _ = geometry.midpoint_grid(self.T, N)
        om_x, om_y = self.broadcast_of(arm, N)
        infid_q, phi_q, _ = qutip_sim.coherent_infidelity(
            t_mid, om_x, om_y, self.T, self.DEV.delta, self.U_TARGET)
        U3 = gate.three_level_propagator(jnp.asarray(om_x), jnp.asarray(om_y),
                                         self.T, self.DEV.delta)
        B = np.asarray(gate.qubit_block(U3))
        _phi_j, fid_j = armcore.fit_phi_vz(B, self.U_TARGET)
        infid_j = 1.0 - fid_j
        return {"N": N, "infid_jax": infid_j, "infid_qutip": float(infid_q),
                "phi_vz_qutip": float(phi_q),
                "rel_diff": abs(infid_q - infid_j) / max(infid_j, 1e-300)}
    ...
    def _make_mc_fn(self, om_x, om_y, phi_vz: float):
        ...
        def one(delta_z, epsilon):
            ...
            U3 = gate.three_level_propagator(om_x * scale, om_y * scale, self.T,
                                             self.DEV.delta, delta_z=delta_z)
    ...
    def checkpoint_trace(self, arm_key: str = "C", N: int | None = None,
        ...
        for k in range(n):
            ...
            om_x, om_y = parametrization.general_waveform(a, c, Phi_0, self.T, self.DEV.delta, N)
        ...
        return {
            ...
            **{f"surrogate_{k}": np.asarray(hist[f"cost_{k}"]) for k in ("c1", "c2", "c3", "c4")},
            ...
            "fluence_over_floor": fluence / self.FLUENCE_FLOOR,
            "max_abs_tau": max_tau,
            "peak_frac": peak / self.DEV.rabi_max_rate,
    ...
    def covariance_check(self, arm_key: str = "B", N: int | None = None) -> dict:
        ...
        src = GateLab(source_spec(self.spec), self.N_DESIGN, self.N_SIM, device=self.DEV)
    ...
    def export_arms(self, arms: dict, rows: dict, outdir: Path | None = None) -> dict:
        ...
        Per-arm JSON contract (N3, ``Novera_work/README.md`` Sec.6): ``omega_x``/
        ``omega_y`` are the played waveform on the export grid; ``phi_vz`` is the
        virtual Z the caller must apply -- the solved object is the qubit-block polar
        decomposition ``W = R_z(phi_vz) . U_target``, so shipping ``(omega_x,
        omega_y)`` without it hands the caller a pulse that is *not* the target gate
        (the convention is spelled out in the JSON itself, not just here). ``device``
        is the full :meth:`curve_opt.device.Device.to_manifest` blob. ``status`` and
        ``warm_start_provenance`` are read back from the arm's own recorded run
        (``_runs/<run_id>/final.json`` / ``manifest.json``) when it has one -- a
        baseline arm (A/B) has no run and reports ``None`` for both, honestly, rather
        than inventing a status for a curve that was never optimized.
        """
        import subprocess

        from curve_opt import recorder as _recorder  # local: only this method needs it
        ...
        manifest = {
            ...
            "device": {"anharmonicity": self.DEV.anharmonicity, "rabi_max": self.DEV.rabi_max,
                       "gate_time": self.DEV.gate_time, "n_modes": self.DEV.n_modes,
                       "static_detuning": self.DEV.static_detuning,
                       "control_error": self.DEV.control_error},
            "arms": {},
        }
        for key in self.ARM_ORDER:
            ...
            qutip_sim.export_waveform_csv(csv_path, t_mid, om_x, om_y)
            infid_120, _phi, _ = qutip_sim.coherent_infidelity(
                t_mid, om_x, om_y, self.T, self.DEV.delta, self.U_TARGET)

            run_id = getattr(arm, "run_id", None)
            status, warm_start_provenance = None, None
            if run_id is not None:
                run_dir = _recorder.RUNS_DIR / run_id
                final_path, manifest_path = run_dir / "final.json", run_dir / "manifest.json"
                if final_path.exists():
                    status = json.loads(final_path.read_text()).get("status")
                if manifest_path.exists():
                    warm_start_provenance = (
                        json.loads(manifest_path.read_text())
                        .get("ansatz", {}).get("warm_start_provenance"))
            ...
            json_path.write_text(json.dumps({
                ...
                "omega_x": np.asarray(om_x).tolist(),
                "omega_y": np.asarray(om_y).tolist(),
                "phi_vz": float(row["phi_vz"]),
                "phi_vz_convention": (
                    "The solved object is the three-level qubit-block polar "
                    "decomposition W = R_z(phi_vz) . U_target. Playing (omega_x, "
                    "omega_y) alone is NOT the target gate -- phi_vz must be applied "
                    "as a virtual Z (frame update) after the pulse."),
                "theta": self.theta, "M": self.DEV.n_modes, "T_ns": self.T,
                "gate_time": self.T,
                "sample_rate": self.DEV.sample_rate,
                "device": self.DEV.to_manifest(),
                "run_id": run_id,
                "status": status,
