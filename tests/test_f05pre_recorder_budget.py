...
def test_three_existing_energy_era_records_are_unaffected():
    run_ids = [r for r in recorder.list_runs()
               if recorder.load(r).manifest.get("schema_version", 1) == 1]
    if len(run_ids) < 3:
        pytest.skip("the tracked _runs/ store carries only budget-schema (v2) "
                    "reference runs; this v1 compatibility check needs the "
                    "exploration repository's historical store")
    rng = random.Random(0)
